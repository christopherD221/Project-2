 ...
 * Created on june 6th, 2020, 9:59 PM
 * Purpose: Battleship
 */
...
void srtScrn(char start, int num){
    ...
    cout<<"The computer will randomly place "<<num<<" boats 1 space big and "
...
void dsplyBd(int array[], int SIZE, int turn, int maxTurn){
    for (int n=0; n<SIZE; n++){
        //Displays heading of gameboard
        if(n==0){
            cout<<"Turn#: "<<turn<<"                     ";
            cout<<"Max Turn#: "<<maxTurn<<endl;
            cout<<setw(27)<<"Computer Board"<<endl;
        }
        //Outputs Values of gameboard and accounts for hits and misses
        if(array[n]==0)
            cout<<"   "<<'x';
        else if(array[n]=='o')
            cout<<"   "<<'o';
